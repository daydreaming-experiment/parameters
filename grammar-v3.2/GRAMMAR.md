...
4. A *details* object is the body of the question description; its content depend on the `type` property of the question:
  ...
  * If `type` is `"slider"` or `"starRating"`:
    ...
    * sub-questions have the following *optional* properties:
      ...
      * `alreadyValid`: a *boolean*; if `true`, the sub-question is considered already answered when it's shown to the user: they can change the value, but if they don't, the original value is accepted; defaults to `false`.
...
5. A *sequence definition* object is used to define sequences, which represent any list of questions, with additional positioning parameters. Such an object has the following fields, all mandatory except otherwise noted:
  ...
  * `showProgressHeader`: an optional *boolean* defining if progress information should be shown at the top of the pages in the sequence; defaults to *true*.
